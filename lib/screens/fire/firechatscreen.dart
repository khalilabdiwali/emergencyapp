...
import 'package:sms/screens/fire/FireOffline.dart';
...
enum UserRole { regular, fire }

class FireChatScreen extends StatefulWidget {
  @override
  _FireChatScreenState createState() => _FireChatScreenState();
}

class _FireChatScreenState extends State<FireChatScreen> {
  ...
  Future<void> _getUser() async {
    ...
    if (user != null) {
      setState(() {
        userId = user.uid;
        userRole =
            user.email == "fire@example.com" ? UserRole.fire : UserRole.regular;
      });
    }
  }
      ...
      if (snapshot.snapshot.value != null && snapshot.snapshot.value is Map) {
        ...
        setState(() {
          ...
          values.forEach((key, value) {
            ...
            if (value['role'] == 'fire' && userRole == UserRole.regular) {
              // Only allow regular users to see fire users
              userList.add(userData);
            } else if (value['role'] == 'regular' &&
                userRole == UserRole.fire) {
              // Only allow fire users to see regular users
              userList.add(userData);
            }
          });
        });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Police Department', style: GoogleFonts.nunito()),
      ),
      body: Padding(
              ...
              children: [
                ListTile(
                  ...
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(builder: (context) => FireOffline()),
                    );
                  },
                              ...
                              Container(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    // Uncomment and update the following lines if you want to handle location messages
                                    ...
                                    //   ),
                                    // if (message['type'] == 'location')
                                    //   _buildLocationMessage(message, isSentByMe, theme),
                                    SizedBox(height: 5),
                                    Text(
                                      formattedTime,
                                      style: TextStyle(
