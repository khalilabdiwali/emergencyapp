...
import 'package:sms/screens/traffic/TrafficOffline.dart';
...
class TrafficChat extends StatefulWidget {
  @override
  _TrafficChatState createState() => _TrafficChatState();
}

class _TrafficChatState extends State<TrafficChat> {
  ...
  Future<void> _getUser() async {
    ...
    if (user != null) {
      setState(() {
        userId = user.uid;
        userRole = user.email == "traffic@example.com"
            ? UserRole.traffic
            : UserRole.regular;
      });
    }
      ...
      if (snapshot.snapshot.value != null && snapshot.snapshot.value is Map) {
        ...
        setState(() {
          ...
          values.forEach((key, value) {
            ...
            if (value['role'] == 'traffic' && userRole == UserRole.regular) {
              // Only allow regular users to see traffic users
              userList.add(userData);
            } else if (value['role'] == 'regular' &&
                userRole == UserRole.traffic) {
              // Only allow traffic users to see regular users
              userList.add(userData);
            }
          });
        });
